...
@interface ARViewController ()<ARSCNViewDelegate,ARSessionDelegate,UIPopoverControllerDelegate,VirtualObjectSelectionViewControllerDelegate,UIPopoverPresentationControllerDelegate>
    ...
    switch (camera.trackingState) {
        ...
        case ARTrackingStateLimited:
            if (self.use3DOFTrackingFallback) {
                ...
                [self.textManager escalateFeedbackForARCameraTrackingState:camera InSeconds:10.0];
...
- (PositionModel *)worldPositionFromScreenPosition:(CGPoint)position ObjectPostion:(SCNVector3)objectPosition InfinitePlane:(BOOL)infinitePlane{
    ...
    if(planeHitTestResult.firstObject){
        ...
        return [PositionModel modelWithPosition:planHitTestPosition PlaneAnchor:(ARPlaneAnchor *)planeAnchor HitAPlane:YES];
    }
    ...
    if ((infinitePlane && self.dragOnInfinitePlanesEnabled)||!highQualityFeatureHitTestResult) {
        ...
        if (pointOnInfinitePlane.x!=0 && pointOnInfinitePlane.y!=0 && pointOnInfinitePlane.z!=0 ) {
            return [PositionModel modelWithPosition:pointOnInfinitePlane PlaneAnchor:nil HitAPlane:YES];
        }
    }
    if (highQualityFeatureHitTestResult) {
        return [PositionModel modelWithPosition:featureHitTestPosition PlaneAnchor:nil HitAPlane:NO];
    }
    ...
    if (unfilteredFeatureHitTestResults.count!=0) {
        ...
        return [PositionModel modelWithPosition:result.position PlaneAnchor:nil HitAPlane:false];
    }
    return [PositionModel modelWithPosition:SCNVector3Zero PlaneAnchor:nil HitAPlane:NO];
}
#pragma mark ---------Plane
- (void)addPlaneWithNode:(SCNNode *)node Anchor:(ARPlaneAnchor *)anchor{
    ...
    [self.textManager showDebugMessage:[NSString stringWithFormat:@"NEW SURFACE DETECTED AT %@",[SCNVector3Tool friendlyStringWithSCNVector3:position]]];
...
- (void)restartPlaneDetection{
    ...
    if ([self.sessionCfg isKindOfClass:[ARWorldTrackingSessionConfiguration class]]) {
        ARWorldTrackingSessionConfiguration *session = (ARWorldTrackingSessionConfiguration *)self.sessionCfg;
        session.planeDetection = ARPlaneDetectionHorizontal;
        self.sessionCfg = session;
    }
    [self.session runWithConfiguration: self.sessionCfg options: ARSessionRunOptionResetTracking|ARSessionRunOptionRemoveExistingAnchors];
...
//    addObjectButton.setImage(#imageLiteral(resourceName: "buttonring"), for: [])
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [object loadModel];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.focusSquare.lastPosition.x == 0 && self.focusSquare.lastPosition.y == 0 && self.focusSquare.lastPosition.z == 0) {
...
- (IBAction)restartExperience:(id)sender {
    if (!self.restartExperienceButtonIsEnabled&&self.isLoadingObject) {
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateFocusSquare{
    //获取屏幕中心点
    if (self.screenCenter.x == 0 && self.screenCenter.y == 0 ) {
        return;
    }
    ...
    PositionModel *model = [self worldPositionFromScreenPosition:self.screenCenter ObjectPostion:self.focusSquare.position InfinitePlane:NO];
    if (model.position.x!=0&&model.position.y!=0&&model.position.z!=0) {
        [self.focusSquare updateForPosition:model.position PlaneAnchor:model.planeAnchor Camera:self.session.currentFrame.camera];
...
- (void)setShowHitTestAPIVisualization:(BOOL)showHitTestAPIVisualization{
    _showHitTestAPIVisualization = showHitTestAPIVisualization;
    if (showHitTestAPIVisualization) {
        self.hitTestVisualization = [[HitTestVisualization alloc]initWithSceneView:self.sceneView];
    }else{
        self.hitTestVisualization = nil;
    }
}
#pragma mark ---------LAZY
- (ARSession *)session{
